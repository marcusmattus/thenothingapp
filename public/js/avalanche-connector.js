...
class AvalancheConnector {
  ...
  init() {
    try {
      ...
      // Create read-only fallback for when no wallet is available
      else {
        console.log('No Ethereum wallet detected. Setting up read-only mode.');
        // Use Avalanche public RPC endpoint for read-only functionality
        const fallbackRPC = this.networks.mainnet.rpcUrl;
        this.provider = new Web3.providers.HttpProvider(fallbackRPC);
        this.web3 = new Web3(this.provider);
        this.walletType = 'readOnly';
        this.isReadOnly = true;
        
        // Make web3 globally available for other components
        window.web3 = this.web3;
        
        // Set the current chain ID to Avalanche Mainnet for read-only mode
        this.currentChainId = parseInt(this.networks.mainnet.chainId, 16);
        
        // Trigger connect event with read-only flag
      ...
      console.error('Error initializing Avalanche connector:', error);
      
      // Try to set up a fallback provider for read-only access
      try {
        console.log('Setting up fallback provider after error');
        const fallbackRPC = this.networks.mainnet.rpcUrl;
        this.provider = new Web3.providers.HttpProvider(fallbackRPC);
        this.web3 = new Web3(this.provider);
        this.walletType = 'readOnly';
        ...
        this.triggerEvent('connect', { 
          account: null, 
          readOnly: true,
          chainId: parseInt(this.networks.mainnet.chainId, 16)
        });
