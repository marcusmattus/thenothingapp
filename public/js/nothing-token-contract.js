...
class NothingToken {
  ...
  async init(web3) {
    try {
      // Use provided web3 instance or try to find one
      if (web3) {
        this.web3 = web3;
        console.log('Using provided Web3 instance for token contract');
      } else if (window.web3) {
        this.web3 = window.web3;
        console.log('Using window.web3 for token contract');
      } else if (window.ethereum) {
        this.web3 = new Web3(window.ethereum);
        console.log('Using window.ethereum for token contract');
      } else {
        // Create a fallback provider for read-only access
        console.log('No Web3 provider found, using fallback for read-only access');
        const fallbackRPC = 'https://api.avax.network/ext/bc/C/rpc';
        this.web3 = new Web3(new Web3.providers.HttpProvider(fallbackRPC));
        this.isReadOnly = true;
      }
      
      // Set default token details (in case contract loading fails)
      this.tokenDetails = {
        name: 'Nothing Token',
        symbol: 'NTH',
        decimals: 18,
      ...
      this.contract = new this.web3.eth.Contract(NothingTokenABI, this.contractAddress);
      
      // If available, try to get actual token details from the contract
      try {
        await this._loadTokenDetails();
        console.log('Successfully loaded token details from contract');
      } catch (error) {
        console.warn('Could not load token details from contract, using defaults:', error);
      }
      
      // Initialize token state for burn/buy/sell operations
      this._initializeTokenState();
      
      return this.contract;
    } catch (error) {
      console.error('Failed to initialize token contract:', error);
      this._createFallbackContract();
      return this.contract;
    }
  }
  
  /**
   * Create a fallback contract when initialization fails
   * @private
   */
  _createFallbackContract() {
    console.log('Creating fallback token contract');
  ...
  /**
   * Initialize token state for operations
   * @private
   */
  _initializeTokenState() {
    // Set initial state for token operations
    if (window.tokenState) {
      // Sync with existing token state if available
      window.tokenState.totalSupply = this.tokenDetails.maxSupply;
      window.tokenState.circulatingSupply = this.tokenDetails.circulatingSupply;
      window.tokenState.burnedTokens = this.tokenDetails.burned;
    } else {
      // Create new token state
      window.tokenState = {
        totalSupply: this.tokenDetails.maxSupply,
        circulatingSupply: this.tokenDetails.circulatingSupply,
        burnedTokens: this.tokenDetails.burned,
        price: 0.01, // Default price in AVAX
        priceHistory: []
      };
    }
  }
